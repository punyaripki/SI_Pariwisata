@extends('layouts/contentNavbarLayout')

@section('title', 'Input groups - Forms')

@section('content')
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Forms /</span> Input Wisata</h4>

    <div class="row">
        <!-- Basic -->
        <div class="col-md-6">
            <div class="card mb-4">
                <form action="{{ route('hotelentry') }}" method="POST">
                    @csrf
                    <h5 class="card-header">Basic</h5>
                    <div class="card-body demo-vertical-spacing demo-only-element">
                        <div>
                            <label for="defaultFormControlInput" class="form-label">id hotel</label>
                            <input type="text" name="id_hotel" class="form-control" id="defaultFormControlInput"
                                placeholder="" aria-describedby="defaultFormControlHelp" />

                        </div>
                        <div>
                            <label for="defaultFormControlInput" class="form-label">Name hotel</label>
                            <input type="text" name="nama_hotel" class="form-control" id="defaultFormControlInput"
                                placeholder="" aria-describedby="defaultFormControlHelp" />

                        </div>
                        <div>
                            <label for="defaultFormControlInput" class="form-label">Penilaian</label>
                            <select class="form-control " name="id_penilaian">
                                <option class="" value="">- Silahkan Pilih -</option>
                                @foreach ($penilaian as $items)
                                    <option value="{{ $items->id_penilaian }}">
                                        {{ $items->id_penilaian }}
                                    </option>
                                @endforeach

                            </select>
                            <div>
                                <label for="defaultFormControlInput" class="form-label">Jenis hotel</label>
                                <select class="form-control " name="id_jenishotel">
                                    <option class="" value="">- Silahkan Pilih -</option>
                                    @foreach ($jenishotel as $items)
                                        <option value="{{ $items->id_jenishotel }}">
                                            {{ $items->id_jenishotel }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>


                            <div>
                                <label for="defaultFormControlInput" class="form-label">Alamat hotel</label>
                                <input type="text" name="alamat_hotel" class="form-control" id="defaultFormControlInput"
                                    placeholder="" aria-describedby="defaultFormControlHelp" />
                            </div>

                            <div>
                                <label for="defaultFormControlInput" class="form-label">No Hp</label>
                                <input type="text" name="no_hp" class="form-control" id="defaultFormControlInput"
                                    placeholder="" aria-describedby="defaultFormControlHelp" />
                            </div>
                            <div>
                                <label for="defaultFormControlInput" class="form-label">Banyak Kamar</label>
                                <input type="text" name="banyak_kamar" class="form-control" id="defaultFormControlInput"
                                    placeholder="" aria-describedby="defaultFormControlHelp" />
                            </div>
                            <div>
                                <label for="defaultFormControlInput" class="form-label">No kamar</label>
                                <select class="form-control " name="no_kamar">
                                    <option class="" value="">- Silahkan Pilih -</option>
                                    @foreach ($kamar as $items)
                                        <option value="{{ $items->no_kamar }}">{{ $items->no_kamar }}</option>
                                    @endforeach

                                </select>
                            </div>

                            <div class="card-body">

                                <button type="submit" class="btn rounded-pill btn-primary">Add</button>


                            </div>

                        </div>
                </form>
            </div>
        </div>
    </div>


    </div>
    <div class="card">
        <h5 class="card-header">Table</h5>
        <div class="table-responsive text-nowrap">
            <table class="table">
                <a href="{{ route('pdfhotel') }}" class="btn btn-primary ms-2">Cetak Data</a>
                <thead>
                    <tr>
                        <th>No</th>
                        <th>id_hotel</th>
                        <th>Penilaian</th>
                        <th>jenis Hotel</th>
                        <th>nama Hotel</th>
                        <th>Alamat</th>
                        <th>No hp</th>
                        <th>Banyak kamar</th>
                        <th>No kamar</th>
                        <th>action</th>
                    </tr>
                </thead>
                <tbody class="table-border-bottom-0">
                    @foreach ($hotel as $row)
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $row->id_hotel }}</td>
                        <td>{{ $row->id_penilaian }}</td>
                        <td>{{ $row->id_jenishotel }}</td>
                        <td>{{ $row->nama_hotel }}</td>
                        <td>{{ $row->alamat_hotel }}</td>
                        <td>{{ $row->no_hp }}</td>
                        <td>{{ $row->banyak_kamar }}</td>
                        <td>{{ $row->no_kamar }}</td>
                        </tr>
                        <td>
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <button data-bs-target="#target{{ $row->id_hotel }}" type="button" data-bs-toggle="modal"
                                    class="btn btn-primary btn-sm">Edit</button>
                                <button type="button"
                                    onclick="window.location='{{ route('hotel-destroy', $row->id_hotel) }}'"
                                    class="btn btn-primary btn-sm">Delete</button>
                            </div>
                        </td>
                        </tr>
                        {{-- Modal Edit --}}
                        <div class="modal fade" id="target{{ $row->id_hotel }}" data-bs-backdrop="static"
                            data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel"
                            aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="staticBackdropLabel">Edit Data
                                        </h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form action="/crud/hotel/edit/{{ $row->id_hotel }}" method="POST">
                                            @csrf
                                            <div class="card-body demo-vertical-spacing demo-only-element">
                                                <div>
                                                    <label for="defaultFormControlInput" class="form-label">id
                                                        hotel</label>
                                                    <input type="text" name="id_hotel" class="form-control"
                                                        id="defaultFormControlInput" placeholder=""
                                                        aria-describedby="defaultFormControlHelp"
                                                        value="{{ $row->id_hotel }}" />

                                                </div>
                                                <div>
                                                    <label for="defaultFormControlInput" class="form-label">Name
                                                        hotel</label>
                                                    <input type="text" name="nama_hotel" class="form-control"
                                                        id="defaultFormControlInput" placeholder=""
                                                        aria-describedby="defaultFormControlHelp"
                                                        value="{{ $row->nama_hotel }}" />

                                                </div>
                                                <div>
                                                    <label for="defaultFormControlInput"
                                                        class="form-label">Penilaian</label>
                                                    <select class="form-control " name="id_penilaian">
                                                        <option class="" value="">- Silahkan Pilih -</option>
                                                        @foreach ($penilaian as $items)
                                                            <option value="{{ $items->id_penilaian }}">
                                                                {{ $items->id_penilaian }}
                                                            </option>
                                                        @endforeach

                                                    </select>
                                                </div>
                                                <div>
                                                    <label for="defaultFormControlInput" class="form-label">Jenis
                                                        hotel</label>
                                                    <select class="form-control " name="id_jenishotel">
                                                        <option class="" value="">- Silahkan Pilih -</option>
                                                        @foreach ($jenishotel as $items)
                                                            <option value="{{ $items->id_jenishotel }}">
                                                                {{ $items->id_jenishotel }}
                                                            </option>
                                                        @endforeach

                                                    </select>
                                                </div>
                                                <div>
                                                    <label for="defaultFormControlInput" class="form-label">Alamat
                                                        hotel</label>
                                                    <input type="text" name="alamat_hotel" class="form-control"
                                                        id="defaultFormControlInput" placeholder=""
                                                        aria-describedby="defaultFormControlHelp"
                                                        value="{{ $row->alamat_hotel }}" />
                                                </div>
                                                <div>
                                                    <label for="defaultFormControlInput" class="form-label">No Hp</label>
                                                    <input type="text" name="no_hp" class="form-control"
                                                        id="defaultFormControlInput" placeholder=""
                                                        aria-describedby="defaultFormControlHelp"value="{{ $row->no_hp }}" />
                                                </div>
                                                <div>
                                                    <label for="defaultFormControlInput" class="form-label">Banyak
                                                        Kamar</label>
                                                    <input type="text" name="banyak_kamar" class="form-control"
                                                        id="defaultFormControlInput" placeholder=""
                                                        aria-describedby="defaultFormControlHelp"
                                                        value="{{ $row->banyak_kamar }}" />
                                                </div>
                                                <div>
                                                    <label for="defaultFormControlInput" class="form-label">No
                                                        kamar</label>
                                                    <select class="form-control " name="no_kamar">
                                                        <option class="" value="">- Silahkan Pilih -</option>
                                                        @foreach ($kamar as $items)
                                                            <option value="{{ $items->no_kamar }}">{{ $items->no_kamar }}
                                                            </option>
                                                        @endforeach

                                                    </select>
                                                </div>

                                                <div class="card-body">

                                                    <button type="submit"
                                                        class="btn rounded-pill btn-primary">Edit</button>

                                                </div>

                                            </div>
                                        </form>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary"
                                            data-bs-dismiss="modal">Close</button>
                                        <button type="button" class="btn btn-primary">Understood</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
@endsection
