...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="card mb-4">
                <form action="{{ route('fasilitaskamarentry') }}" method="POST">
                    @csrf
                    <h5 class="card-header">Basic</h5>
                    <div class="card-body demo-vertical-spacing demo-only-element">
                        <div>
                            <label for="defaultFormControlInput" class="form-label">id fasilitas kamar</label>
                            <input type="text" name="id_fasilitaskamar" class="form-control" id="defaultFormControlInput"
                                placeholder="" aria-describedby="defaultFormControlHelp" />

                        </div>
                        <div>
                            <label for="defaultFormControlInput" class="form-label">Keteranagan</label>
                            <input type="text" name="keterangan_fasilitas" class="form-control"
                    ...
                    @foreach ($fasilitaskamar as $row)
                        ...
                        <td>
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <button data-bs-target="#target{{ $row->id_fasilitaskamar }}" type="button"
                                    data-bs-toggle="modal" class="btn btn-primary btn-sm">Edit</button>
                                <button type="button"
                                    onclick="window.location='{{ route('fasilitaskamar-destroy', $row->id_fasilitaskamar) }}'"
                                    class="btn btn-primary btn-sm">Delete</button>
                            </div>
                        </td>
                        </tr>
                        {{-- Modal Edit --}}
                        <div class="modal fade" id="target{{ $row->id_fasilitaskamar }}" data-bs-backdrop="static"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="/crud/fasilitaskamar/edit/{{ $row->id_fasilitaskamar }}"
                                            method="POST">
                                            @csrf
                                            <div class="card-body demo-vertical-spacing demo-only-element">
                                                <div>
                                                    <label for="defaultFormControlInput" class="form-label">id
                                                        fasilitas kamar</label>
                                                    <input type="text" name="id_fasilitaskamar" class="form-control"
                                                        id="defaultFormControlInput" placeholder=""
                                                        aria-describedby="defaultFormControlHelp"
                                                        value="{{ $row->id_fasilitaskamar }}" />
